...
export class BillingService {
  ...
  async upsertBillingInfo(tenantId: string, role: Role, dto: UpdateBillingInfoDto): Promise<BillingInfoResponseDto> {
  ...
  async setDefaultPaymentMethod(tenantId: string, role: Role, id: string): Promise<PaymentMethodResponseDto> {
    this.ensureBillingAdminRole(role);
    await this.ensurePaymentMethod(tenantId, id);
  ...
  async deletePaymentMethod(tenantId: string, role: Role, id: string): Promise<void> {
    this.ensureBillingAdminRole(role);
  ...
  private ensureBillingAdminRole(role: Role): void {
    if (role !== Role.ADMIN && role !== Role.SUPER_ADMIN) {
      throw new ForbiddenException('Insufficient permissions to manage billing');
    }
  }
}
